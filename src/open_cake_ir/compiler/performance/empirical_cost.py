...
class EmpiricalCostModel:
    ...
    def __init__(self, document: object) -> None:
        ...
        if type(row["schema_version"]) is not int or row["schema_version"] != 3:
            raise ValueError("empirical cost model requires schema_version 3 with the Compiler commit identity")
    ...
    def estimate(self, schedule: dict, *, compiler_revision_id: str, target: str,
        ...
        if compiler_revision_id != self.compiler_revision_id or target != self.target:
            result["reason"] = (
                "model Compiler Revision or target differs: model binds "
                f"{self.compiler_revision_id!r} on {self.target!r}, assessed under "
                f"{compiler_revision_id!r} on {target!r}"
            )
            return result
