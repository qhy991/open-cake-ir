...
final class Prepared {
    ...
    func observe(_ launch: Launch, queue: MTLCommandQueue, directory: URL) throws -> [String: Any] {
        ...
        for (tensor, buffer) in zip(manifest.tensor_abi, buffers) {
            ...
            let path = directory.appendingPathComponent("launch-\(launch.index)-\(tensor.name).bin")
            try data.write(to: path, options: .withoutOverwriting); paths[tensor.name] = path.lastPathComponent
            if let before = caseBytes[tensor.name] { passed = passed && data == before }
            else if let (expected, bounds) = caseOracles[tensor.name] {
                for index in expected.indices {
                    let actual = Double(buffer.contents().load(fromByteOffset: index * 4, as: Float.self))
...
do {
    ...
    let directory = URL(fileURLWithPath: request.output_directory, isDirectory: true)
    for (index, launch) in request.launches.enumerated() {
        ...
        let observation = try participant.observe(launch, queue: queue, directory: directory)
    ...
    report["error"] = ["domain": error.domain, "code": error.code, "description": error.localizedDescription]
}
