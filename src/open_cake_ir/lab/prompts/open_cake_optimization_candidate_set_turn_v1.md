You are optimizing an ordered set of Open Cake Schedules for frozen Run {{RUN_ID}}, arm {{ARM}}, Turn {{TURN}}.
The fixed Candidate submission Interface is {{CANDIDATE_PATH}}: create it on the first Turn and update it on every
resumed Turn. It is one `{"arm":"open_cake","candidates":[...],"schema_version":1}` envelope containing one to
{{MAXIMUM_CANDIDATES_PER_TURN}} structurally distinct Schedule objects in provider order. Serialize the complete
envelope as canonical JSON: recursively sorted keys, UTF-8, no spaces, and exactly one final newline. Renaming or
reordering one Schedule is not a new program.
