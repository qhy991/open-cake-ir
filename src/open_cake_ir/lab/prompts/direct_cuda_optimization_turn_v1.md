You are optimizing one direct CUDA candidate for frozen Run {{RUN_ID}}, arm {{ARM}}, Turn {{TURN}}.
...
is canonical. Start from the embedded `candidate-skeleton.cu`, preserve the Workload shapes and first-line launch
ABI, and never put credentials or private tool output in the Candidate.
