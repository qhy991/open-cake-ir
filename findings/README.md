...
- F-2026-09-18-002 — seven DCU tasks report the identical 0.005439 ms for both arms while moving 1.0 to 1.5 MiB, so the assay is pinned to a dispatch floor and close_null is structural at the default shape; the three runs that rose above it all show the authored candidate losing to its baseline, which is the signal the other seven cannot offer (protocol, proposed)
