...
- F-2026-09-18-001 — five vendor-neutrality defects AGENTS.md tracked as prose are still open after that ledger was rewritten as invariants: `BINARY_VERSION != 100` and a B200 shared-memory literal in the test-only `launch_cubin_once` chain, an arm-kind refusal worded in CUDA's words, `Role.warps` as the only spelling of a shared concept, two NVIDIA mnemonics in `PLACED_CONTRACTS`, and warp-width literals in the Metal and native CUDA emitters (bug, proposed; the `warps` rename is authorized under D4 as a hash-only Phase 5 tick since `calibration_coverage` has been empty in every commit and lock, the rest are Phase 1 items)
