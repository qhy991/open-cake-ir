...
- F-2026-09-15-003 — the AMDGCN Evaluation half exists in parts (HIP host admission, exact-HIP runtime custody, rocprofv3 trace projection) but no gfx target has an executable role, no timing path is named for a trace, and schema v2 identity pins gfx1151 while the Compiler target is gfx938 (capacity, proposed; two earlier observations in this record were read from a stale ref and are corrected in it)
