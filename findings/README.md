...
- F-2026-09-16-002 — the Compiler already lowers the SoL-ExecBench FP16 GEMM shape (C = A @ B.T, FP16 in and out) to CUDA on sm_103a via `tcgen05.mma...kind::f16`; the eight FIB GEMM captures are blocked by three layers above it — the Workload ABI cannot name fp16, `tasks/devices.py` declares no native_cuda row, and `lab/` has no native-CUDA ToolchainBuilder — while the Triton route separately declares no FP16 dot on either NVIDIA Target (capacity, proposed; source probes only, no campaign and no measurement)
- F-2026-09-16-001 — a kimi-k3 experiment service runs B300 tasks end to end; twelve of thirteen campaigns adhered, custody-verified and semantically replayed, and the refused one invoked Bash, a tool the authoring environment does not offer (behavior, artifact_optimization_only; also: the launcher's default gpu-run socket is not this host's, and it cost a full authoring turn to find out)
