...
- F-2026-09-16-001 — the subscription gateway serves sustained transport-failure windows with no rate-limit notice at all; those deaths are distinguishable from the quota wall only by observed_quota being absent, and a turn-1-gated requeue recovered all fifteen overnight deaths under the same frozen Revision (behavior, proposed)
- F-2026-09-16-002 — at a hard run boundary the final turn declares candidate_written true with no witnessed write; the broker correctly refuses, but the fault overwrites a terminal state the checkpoint had already settled (protocol, 4 instances across token-budget and five-hour boundaries; proposed)
...
- F-2026-09-17-009 — gfx938 declared `mma` and two dot contracts that no Corpus case reached, so the Gate passed 151/151 with the declaration present or absent (capacity, closed by two cases: one lowering to `tl.dot` over fp16 operands, one pinning the operand-dtype refusal that had been fail-open; Gate 153/153)
...
- F-2026-09-18-002 — eleven DCU tasks report the identical 0.005439 ms for both arms while moving 1.0 to 1.5 MiB, so the assay is pinned to a dispatch floor and close_null is structural at the default shape; the three runs that rose above it all show the authored candidate losing to its baseline, which is the signal the other seven cannot offer (protocol, proposed)
