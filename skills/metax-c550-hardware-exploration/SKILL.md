...
For the fixed 64×64 FP8 GEMM, use the [frozen Workload](../../contracts/workloads/metax-fp8-e4m3-gemm-fp32-xcore1002-m64-n64-k64-v1.json) as the semantic and oracle owner. It adds deterministic finite E4M3FN inputs, an independent high-precision reference, and held-out full-range/bounded distributions. Its `heldout_full_finite_05` case contains an ordinary FP32 sequential-sum tolerance failure; that is a numerical counterexample, not evidence that any particular Cake reduction has the same output. Requalify the admitted compensated Cake source and any explicit-precision successor against every Workload case before comparing their timing. The pre-Workload standalone FP64 receipts remain diagnostic evidence tied to their old inputs.
