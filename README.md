...
- **想在 Apple M2 上运行：** [Metal 入门](docs/metal.zh-CN.md) · [English guide](docs/metal.md)，涵盖 FP32 逐元素计算与行归约。
...
Metal 目前是基础正确性原型：`serial_program_tile` 由每个线程组的一个活动线程串行处理完整 tile。
此路径连接 Compiler 与直接运行适配器；Metal 的 Lab/Campaign、Agent 搜索和性能优化尚未提供。
