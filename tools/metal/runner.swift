...
func loadManifest(_ manifestPath: String) throws -> (Manifest, [Data?]) {
    ...
    try require(["apple_gpu_family7", "apple_gpu_family8", "apple_gpu_family9"].contains(manifest.target) && manifest.source_language == "metal" &&
...
final class Prepared {
    ...
    init(path: String, device: MTLDevice) throws {
        ...
        let exactDevice = manifest.target == "apple_gpu_family7" ? "Apple M1 Pro"
            : manifest.target == "apple_gpu_family8" ? "Apple M2" : "Apple M4"
        let exactFamily = manifest.target == "apple_gpu_family7"
            ? device.supportsFamily(.apple7) && !device.supportsFamily(.apple8)
            : manifest.target == "apple_gpu_family8"
...
func execute() throws {
    ...
    receipt["target"] = device.name == "Apple M1 Pro" ? "apple_gpu_family7"
        : device.name == "Apple M2" ? "apple_gpu_family8"
        : device.name == "Apple M4" ? "apple_gpu_family9" : "unsupported"
