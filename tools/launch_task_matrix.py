...
first provider call, then reused by their tasks. The first successful provider qualification is reused under the
...
def _command(args, task: str, workspace: Path, qualification: tuple[Path, Path] | None,
             baseline: Path | None = None) -> list[str]:
    ...
    if baseline is not None:
        command.extend(("--fixed-baseline-bundle", str(baseline)))
    return command
...
def main(argv=None) -> int:
    ...
    failures = 0
    stopped = False
    # Admit every baseline before any provider tokens. Reuse the sealed bundle in
    # the actual task; this is the same build path as the single-task launcher.
    baselines = {}
    for position, task in enumerate(selected, start=1):
        ...
        _append(root / "baseline-results.jsonl", {"task": task,
            "exit_code": prepared.returncode, "workspace": str(preparation)})
        if prepared.returncode != 0:
            _write(root / "terminal.json", json.dumps({"schema_version": 1,
                "status": "stopped_before_baseline_preflight", "failed_task": task,
                "task_count_requested": len(selected), "task_count_attempted": 0,
                "provider_calls": 0, "finished_at": _now()}, indent=2).encode() + b"\n")
            return 1
        # The canonical producer prints its external sealed bundle locator last.
        baseline = Path(prepared.stdout.decode().strip().splitlines()[-1])
        baselines[task] = launch_task.external_file(ROOT, str(baseline), "prepared baseline")
    for position, task in enumerate(selected, start=1):
        workspace = root / task
        started = _now()
