# Python candidate bundle optimization — Metal scaffold v1
...
Python only; write ordered complete `@cake.schedule(...)` functions in
`candidate-set.py`. A granted rewrite may be declared with static
...
stopping. Do not invoke those tools, a GPU, the network or another task. Change only
`candidate-set.py`; keep TASK.md and AGENTS.md unchanged. Do not author serialized
Schedule JSON or insert a low-level baseline.

This Metal source-bundle scaffold is a separate authoring treatment from v4. Keep its Runs separate;
