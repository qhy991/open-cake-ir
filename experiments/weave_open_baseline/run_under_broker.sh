...
set -euo pipefail
if [[ $# != 3 ]]; then
  echo "usage: $0 <upstream-checkout> <isolated-build-dir> <output-dir>" >&2
  exit 2
fi
...
if [[ ! -f $output_root/admission.json || -e $output_root/device-observation.json ]]; then
...
docker run --rm --network host --ipc host \
  ...
  -v "$adapter_root:/adapter:ro" -v "$output_root:/out" -w /adapter \
  lmsysorg/sglang:latest-cu130-runtime \
  /build/venv/bin/torchrun --standalone --nproc_per_node=4 \
  /adapter/runner.py run --upstream /src --output /out
