...
def _load_workload(project_root: Path) -> WorkloadContract:
    workload = load_workload(project_root / WORKLOAD)
...
def _evaluate_cases(
    ...
    for case_id in workload.case_ids:
        ...
        input_unchanged = (
            ...
            and bool(torch.equal(x.view(torch.uint8), x_before.view(torch.uint8)))
            and bool(torch.equal(gamma.view(torch.uint8), gamma_before.view(torch.uint8)))
        )
        output_contract = (
...
def _run_live_impl(
    ...
    torch, properties = _admit_device(project_root)
