...
def _validate_lowering(project_root: Path, requirements: Mapping[str, object]) -> None:
    target = Target.load(project_root / "compiler/targets/gfx1151.json")
    if (
        requirements.get("target") != target.target_id
        or requirements.get("triton_target") != target.triton_target
        or {key: value for key, value in requirements.items()
            if key not in {"target", "triton_target"}} != _PRODUCER_ABI
    ):
...
def _prepare(
    ...
    _validate_lowering(project_root, requirements)
...
def _run_live_impl(
    ...
    try:
        ...
        for case_id in workload.case_ids:
            ...
            activation_pointer = int(activation.data_ptr())
            output = torch.full(
                (16, 36),
                0xA5,
                dtype=torch.uint8,
            ...
            activation_unchanged = (
                int(activation.data_ptr()) == activation_pointer
                and bool(torch.equal(
                    activation.view(torch.uint8), activation_before.view(torch.uint8)
                ))
            )
