...
Raw output of the instruments in `tools/` that test the static analysis against a device. `docs/ANALYSIS_CALIBRATION.md` is where these are read; this is what
it is reading, so a claim there can be checked rather than taken.
...
The ranking-calibration measurements (`rmsnorm-b200-ranking-*`, `gemm-b200-ranking-m512-v5`
to `-v8-*`, `flash-kmeans-b200-ranking-n512-v5`) and the three frozen plans under
`contracts/calibrations/` live on the `history` branch (`git show history:evidence/calibration/<file>`)
together with the instruments that produced them; the negative result they recorded --
three preregistered B200 calibrations failed, `calibration_coverage` stayed empty in every
release -- is the reason ranking is removed from the Compiler rather than kept dormant.
`docs/ANALYSIS_CALIBRATION.md` still reads them by name.
