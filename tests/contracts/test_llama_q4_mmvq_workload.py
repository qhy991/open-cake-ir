...
ORACLE_SOURCE = ROOT / "src/open_cake_ir/evaluation/llama_q4_mmvq.py"
...
class LlamaQ4MmvqWorkloadTests(unittest.TestCase):
    ...
    def test_oracle_source_and_upstream_revision_are_exact(self) -> None:
        ...
        q4_semantics = next(
            ...
            if value.get("path") == "ggml/src/ggml-quants.c"
        )
        self.assertEqual(
            q4_semantics["sha256"],
        ...
        self.assertEqual(
            document["oracle"]["source_sha256"],
            sha256(ORACLE_SOURCE.read_bytes()).hexdigest(),
        )
