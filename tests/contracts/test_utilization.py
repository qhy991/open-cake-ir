...
def _full_peak(flops: float = 2.0e15, bytes_per_second: float = 8.0e12) -> Target:
    return _with_peak(
        memory_bandwidth=_rate(bytes_per_second, "bytes_per_second"),
        arithmetic={DOT: _rate(flops, "flops_per_second")},
    )
...
class UtilizationTest(unittest.TestCase):
    ...
    def test_a_ratio_above_one_reports_as_refuted(self) -> None:
        """No kernel beats the peak, so this is a defect in an input and says so.
...
class AbstentionTest(unittest.TestCase):
    ...
    def test_a_bandwidth_only_peak_yields_a_bandwidth_only_roofline(self) -> None:
        ...
        target = _with_peak(memory_bandwidth=_rate(8.0e12, "bytes_per_second"))
        bound = _bound(GEMM)
