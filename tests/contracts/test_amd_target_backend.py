...
class CompilerTargetCompatibilityTests(unittest.TestCase):
    ...
    def test_schema_v1_b200_hardware_contract_remains_unchanged(self) -> None:
...
class ExactTritonProjectionTests(unittest.TestCase):
    def test_metal_refuses_the_new_reduction_order_instead_of_discarding_it(self):
        from open_cake_ir.compiler.emit_metal import preflight
        document = json.loads((ROOT / "corpus/schedules/metal-row-sum-odd.json").read_text())
        for operation in document["operations"]:
            if operation["kind"] == "reduce":
                operation["parameters"]["algorithm"] = "xor_tree_32"
        target = Target.load(ROOT / "compiler/targets/apple_gpu_family8.json")
        self.assertIn("METAL_REDUCTION_UNSUPPORTED", {finding.code for finding in preflight(Schedule.from_dict(document), target)})

    def test_target_projection_is_shared_and_refuses_gfx_drift(self):
        from open_cake_ir.compiler.emit_triton import preflight
        schedule = Schedule.load(ROOT / "corpus/schedules/swiglu-b8-smoke-gfx1151.json")
        target = Target.from_dict(_gfx1151_target())
        self.assertEqual(target.triton_target, {"backend": "hip", "arch": "gfx1151", "warp_size": 32})
        self.assertIsNone(Target.load(ROOT / "compiler/targets/apple_gpu_family8.json").triton_target)
        for field, value in (("execution_group_width", 64), ("architecture", "gfx1150"), ("compute_capability", [11, 5])):
            document = _gfx1151_target()
            document[field] = value
            drift = Target.from_dict(document)
            self.assertIsNone(drift.triton_target)
            self.assertIn("BACKEND_TARGET_UNSUPPORTED", {finding.code for finding in preflight(schedule, drift)})
