...
class ContractFixture:
    def __init__(self, testcase: unittest.TestCase) -> None:
        ...
        self.compiler = {
            "schema_version": 1,
            "revision_id": "open-cake-ir-test-compiler",
            "state": "released",
        }
        self.executor = {
            "schema_version": 1,
            "executor_id": "open-cake-ir-test-executor",
            "state": "released",
        }
...
class AmdRmsNormSearchContractTests(unittest.TestCase):
    @unittest.skipUnless(
        ...
        "requires a frozen Search Contract bound to an independently approved Compiler and HIP Executor",
    )
    def test_released_contract_prepares_the_complete_domain_without_a_gpu(self) -> None:
        ...
        contract = AmdRmsNormSearchContract.load(ROOT, SEARCH_CONTRACT)
        self.assertEqual(result["compiler"]["revision_id"], contract.compiler_revision_id)
        self.assertEqual(
            result["executor"]["executor_id"], contract.executor_id
        )
    ...
    def test_loads_exact_frozen_contract_and_owned_json(self) -> None:
        ...
        self.assertEqual(contract.compiler_revision_id, "open-cake-ir-test-compiler")
        self.assertEqual(contract.executor_id, "open-cake-ir-test-executor")
