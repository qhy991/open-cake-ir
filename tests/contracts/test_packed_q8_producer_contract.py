...
def _conformance_codes(document: dict[str, object]) -> tuple[str, ...]:
    ...
    if any(
        ...
        != {"to": "fp16", "rounding": "nearest_even", "overflow": "ieee"}
...
class PackedQ8ProducerContractTests(unittest.TestCase):
    ...
    def test_dag_keeps_fp32_scale_source_xor_sum_and_relation_derived_store(self) -> None:
        ...
        self.assertEqual(
            ...
            {"to": "int8", "rounding": "toward_zero", "overflow": "forbid"},
        )
        for name in ("cast_d_fp16", "cast_s_fp16"):
            self.assertEqual(
                operations[name]["parameters"],
    ...
    def test_schedules_use_canonical_cast_and_are_adopted_as_corpus_inputs(self) -> None:
        ...
        self.assertTrue(compiler.assess_file(POSITIVE).lowering_eligible)
        self.assertFalse(compiler.assess_file(RECORD_COUNT_DRIFT).accepted)
