...
from open_cake_ir.lab import core
    ...
    def test_full_preflight_reaches_lock_and_python_package_for_both_harnesses(self):
        for harness in ('codex','claude-code'):
            with self.subTest(harness=harness), tempfile.TemporaryDirectory() as temporary:
                ...
                with patch.object(core,'resolve_execution_bindings',return_value=(study,executor)), \
                     patch.object(core.ProviderQualificationReceipt,'load',return_value=receipt), \
                     patch.object(core,'load_baseline_bundle',return_value=candidate):
