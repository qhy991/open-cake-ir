...
class LabContractTests(unittest.TestCase):
    ...
    def test_current_study_successors_bind_the_current_executor(self) -> None:
        for name in (
            "matched-search-infrastructure-v32.json",
            "matched-search-system-qualification-v32.json",
            "artifact-optimization-v32.json",
            "flash-kmeans-r45-portfolio-reconstruction-v32.json",
            "matched-search-clean-start-reference-v32.json",
        ):
    ...
    def test_v25_retains_the_reference_bundle_or_records_a_missing_endpoint(self) -> None:
        lab = Lab(ROOT)
        lock = lab.preflight(
            ROOT / "contracts/studies/matched-search-system-qualification-v32.json"
        )
    ...
    def test_artifact_optimization_preflight_binds_full_features_without_an_estimand(self) -> None:
        lock = Lab(ROOT).preflight(
            ROOT / "contracts/studies/artifact-optimization-v32.json"
        )
    ...
    def test_closed_provider_receipt_cannot_authorize_artifact_optimization(self) -> None:
        study = json.loads(
            (ROOT / "contracts/studies/artifact-optimization-v32.json").read_text()
        )
    ...
    def test_fixture_provider_qualification_forbids_an_external_anchor(self) -> None:
        study = json.loads(
            (
                ROOT
                / "contracts/studies/matched-search-system-qualification-v32.json"
            ).read_text()
        )
    ...
    def test_preflight_rejects_a_changed_direct_candidate_skeleton(self) -> None:
        study = json.loads(
            (ROOT / "contracts/studies/matched-search-infrastructure-v32.json").read_text()
        )
    ...
    def test_artifact_optimization_promotes_per_run_without_scientific_analysis(self) -> None:
        ...
        lab = Lab(ROOT)
        lock = lab.preflight(ROOT / "contracts/studies/artifact-optimization-v32.json")
        resolved = lock.document["resolved_inputs"]
        protocol_sha256 = sha256(
            json.dumps(
                lock.document["evaluation_protocol"],
    ...
    def test_preflight_rejects_a_draft_compiler_revision(self) -> None:
        study = json.loads((ROOT / "contracts/studies/matched-search-infrastructure-v32.json").read_text())
    ...
    def test_preflight_rejects_an_unknown_event_vocabulary(self) -> None:
        study = json.loads(
            (
                ROOT
                / "contracts/studies/matched-search-infrastructure-v32.json"
            ).read_text()
        )
    ...
    def test_lab_owns_two_turn_resume_budget_evaluation_and_terminal(self) -> None:
        lab = Lab(ROOT)
        lock = lab.preflight(ROOT / "contracts/studies/matched-search-infrastructure-v32.json")
        provider = FakeProvider()
        resolved = lock.document["resolved_inputs"]
    ...
    def test_portfolio_semantic_replay_keeps_correctness_separate_from_timing(self) -> None:
        lab = Lab(ROOT)
        lock = lab.preflight(
            ROOT / "contracts/studies/flash-kmeans-r45-portfolio-reconstruction-v32.json"
        )
...
class CandidateSetFilterTest(unittest.TestCase):
    ...
    def test_the_set_is_built_and_ordered_before_one_is_evaluated(self) -> None:
        ...
        lab = Lab(ROOT)
        source = ROOT / "contracts/studies/matched-search-infrastructure-v32.json"
