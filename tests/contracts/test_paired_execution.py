...
class PairedExecutionTests(unittest.TestCase):
    ...
    def test_stable_single_receipt_replays_only_outside_new_paired_policy(self):
        receipt = self.execute()
        payloads = dict(receipt.artifact_payloads)
        ...
        del legacy['paired_timing']
        validate_receipt_policy(old, legacy, None)
    ...
    def test_external_preflight_binds_stable_study_and_rejects_runtime_or_baseline_drift(self):
        ...
        qualification = json.loads((ROOT / 'contracts/providers/fixture-provider-candidate-set-v1.json').read_bytes())
