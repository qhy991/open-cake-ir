...
源码 `f443a7d0696c5aa381f3f32c130ac140fcdadcd3` 增加了明确绑定 `xcore1002` 的
...
| --- | --- | ---: | --- |
| `primary` | `maca-f3002e404819` | 1 / 1 / 1 | 0 mismatch，输入不变 |
| `tiny` | `maca-122657c04285` | 1 / 1 / 1 | 0 mismatch，输入不变 |
| `index_boundaries` | `maca-1dda8236a85a` | 1 / 1 / 1 | 0 mismatch，输入不变 |
| `repeated_indices` | `maca-16cde2cfa335` | 1 / 1 / 1 | 0 mismatch，输入不变 |

四个收据均被公共 Evaluation 接受，最大绝对误差为 0，零 fallback。编译与设备原始记录
保留在 `c550-1:/root/.local/share/open-cake-ir/metax-c550-20260920/` 的
`indexed-gather-build-f443a7d0-v1/`、`indexed-gather-device-f443a7d0-v1/`，本地外部
证据根另存 `metax-parity-20260920/indexed-gather-f443a7d0-evidence.tar.gz`。
