...
为避免只依据一个形状，在**上卡前**再冻结一个未见的同算子形状
`R=1024,K=512,N=64`，沿用已经选定的 K=64 与 canonical whole-K starter，不再搜索
其他 tile。它的五 case 在首次和独立确认中均零不匹配、计时质量通过；独立确认的
starter/候选为 **106.232/97.593 μs（1.0885×）**，十对均为候选更快、每臂 250
个样本。第二个形状支持**同一 Hygon 目标上的有限形状泛化**，但收益显著小于
`K=1024` 的 2.735×；没有 B300 在此新形状的来源收益，也没有跨算子或模型结论。
预定停止规则与原始收据在 checkout 外的
