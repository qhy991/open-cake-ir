...
准备提交 `1718383669ecace011bb51c935d04be531e1ab41` 已在 Verda 的 Triton 3.7.1 上进行实际离线编译：
四个记录中的仿射候选，加上单值读取、非零起点区间、全局 9 列按 4 列分块三个控制例，
都生成了目标为 `sm_100a` 的 PTX 和 ELF CUBIN。原有普通/嵌套 FMA 的舍入编译回归也通过。
这些是本轮新的编译记录，不替换历史 v41 产物，且没有启动 GPU kernel 或测量性能。

完整 Corpus 为 77/77。CPU 回归覆盖访问形状、地址范围、索引写入、原子操作、现有 IR 与生成器；
本地缺少 Torch 的两组检查改用服务器上已有的环境运行，没有安装或修补旧实验环境。
