...
| `mul` | 乘 | `2×3 → 6` |
| `div` | 除 | `6÷3 → 2` |
| `log2` | FP32 以 2 为底的对数；零产生负无穷 | `8 → 3` |
| `floor_div` | INT32 对正整数常量向下整除 | `-3 // 2 → -2` |
| `remainder` | 与向下整除一致的 INT32 非负余数 | `-3 % 2 → 1` |
| `fma` | 融合乘加 | `2×3+4 → 10` |
...
将声明的 program、program tile、loop 或 loop tile 坐标读成 INT32 寄存器值；
`source="range"` 生成显式起点和长度的整数范围。引用 loop 坐标必须位于该循环内部。
它使地址计算可供普通整数算术使用，不隐式改变访问布局。

## compare

对相同数值类型比较，输出 INT32 的 0/1 谓词。支持 lt/le/eq/ne/gt/ge；
操作数形状一致，或显式使用规范 `[1]` 标量。浮点索引不能冒充整数地址。
