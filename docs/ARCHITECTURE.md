...
therefore reports `false` for that path, while all six emitter-backed profiles report
...
| The filter stage | partial — construction, verifier filtering and semantic deduplication are implemented and live exercised, but Compiler v12 retains no calibrated cost order; eligible candidates retain provider order before `searches_per_turn` selects GPU work |
