...
| Compile → external oracle → GPU timing | correctness is implemented. Historical B200 observations cover eleven pre-v24 emitted Schedule slices. The frozen v24 attempt exposed an out-of-bounds GEMM mask and two observation crashes. The preregistered v25 successor compiled and launched all 14 generated lowerings, retained 12 passes and two GEMM failures under the unchanged `1e-5` threshold, and measured no timing. TinyGEMM2 v2 now pins reproduced inputs, independent oracle and retained parent output, but the current checked-asset launch remains missing |
