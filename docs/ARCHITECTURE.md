...
| Deterministic lowering | `lower` generates for 7 of the 8 admitted profiles through 8 of 9 exact Target/profile implementations: Triton on `sm_100a` for `flash_kmeans_b32_smoke`, `rmsnorm_b8_smoke`, `softmax_b8_smoke`, `layernorm_b8_smoke`, `gemm_bias_b1_smoke` and `swiglu_b8_smoke`; warp-specialized CuTe-DSL on `sm_100a` for `flash_kmeans_assignment_full`; and MSL on `apple_gpu_family9` for `flash_kmeans_b32_smoke`. `tinygemm2_stage4_split_k` still stamps a digest into a checked-in file, and the public result exposes `generated=false` rather than conflating it with emission |
