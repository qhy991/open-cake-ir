...
| Compile → external oracle → GPU timing | correctness is implemented; B200 observations cover nine emitted Schedule slices, including the KDA-derived standalone SwiGLU, Top-K, block-scale contraction and valid-prefix slices. Their correctness observations take no timing and support no performance claim |
...
| The filter stage | partial — construction, verifier filtering and semantic deduplication are implemented and live exercised, but Compiler v19 retains no calibrated cost order; eligible candidates retain provider order before `searches_per_turn` selects GPU work |
