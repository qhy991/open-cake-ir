...
```bash
open-cake-ir lab preflight contracts/studies/matched-search-infrastructure-v22.json \
...
The current checked-in scientific matched contract, `matched-search-infrastructure-v22.json`, uses a zero-GPU fixture
...
estimate requires conditional latency in both arms. The current non-scientific G8 template is
`matched-search-system-qualification-v22.json`; earlier versions remain frozen historical records. Freeze
...
`artifact-optimization-v22.json` is the current zero-GPU contract fixture. The frozen
...
python tools/freeze_live_matched_study.py \
  --project-root . \
  --template contracts/studies/matched-search-system-qualification-v22.json \
  --qualification contracts/providers/<new-live-receipt>.json \
  --qualification-anchor evidence/qualifications/<new-live-anchor>.json \
  --executor runtime/executors/open-cake-ir-b200-v22.json \
