...
```bash
open-cake-ir lab preflight contracts/studies/matched-search-infrastructure-v18.json \
...
The current checked-in scientific matched contract, `matched-search-infrastructure-v18.json`, uses a zero-GPU fixture
provider and intentionally cannot start a live provider. The current non-scientific G8 template is
`matched-search-system-qualification-v18.json`; earlier versions remain frozen historical records. Freeze
...
`artifact-optimization-v18.json` is the current zero-GPU contract fixture. The frozen
...
python tools/freeze_live_matched_study.py \
  --project-root . \
  --template contracts/studies/matched-search-system-qualification-v18.json \
