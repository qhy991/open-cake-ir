...
| G3 | passed | v28 35-case Corpus Gate, released content-bound Revision lock, independently written tmux-window1 approval and 51-source closure; the new reservation-owned indexed store adds no IR vocabulary, all nine ownership near misses are failure-capable, and the earlier 34 cases including both TinyGEMM2 dispositions have zero expectation drift |
...
| G6 | passed | 436 pytest items plus 320 subtests cover authoring-schema/Corpus parity, KDA-derived primitives and ownership negatives, generated-source masks, frozen success and pre-GPU failure retention, TinyGEMM2 materialization and checked-asset semantics, external-approval transitions, evidence replay/custody, candidate-set selection, provider lifecycles, Lab authority, GPU tutorial custody and all three matched Claim Scopes |
