...
| G3 | passed | v14 21-case multi-Target Corpus Gate, released content-bound Revision lock, reviewed approval and 39-source closure; all nineteen v13 observations are unchanged, while the Metal staged-loop sibling and scientific-v3 Triton warp-specialized argmin remain lowering-ineligible |
...
| G6 | passed | 369 Mac-compatible tests + 299 subtests cover Target v1/v2 normalization, exact Target/profile routing, generated MSL compilation, v13 observation compatibility, successor Study custody, authoring-schema/Corpus parity, KDA-derived tanh emission, candidate-set transport/selection/replay, semantic tampering, reference bundles, ranking abstention, attribution, Analysis Plans, tool-rich lifecycles, immutable CLI reports, Claim-Scope routing, external Campaign custody and all three matched Claim Scopes; the separate Apple9 probe passes 6/6, while the pinned B200 Nsight Compute executable custody check remains host-specific |
