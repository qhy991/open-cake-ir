...
| G3 | passed | v16 22-case Corpus Gate, released content-bound Revision lock, reviewed approval and 38-source closure; authoring-schema parity, SwiGLU and Top-K shape drifts, the unsupported KDA `tanh.approx.f32` contract and a Target-admitted wrong-kind instruction are failure-capable regression gates, while the scientific-v3 Triton warp-specialized argmin failure remains lowering-ineligible |
