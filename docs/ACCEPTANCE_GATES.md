...
| G3 | passed | v23 32-case Corpus Gate, released content-bound Revision lock, reviewed approval and 48-source closure; authoring-schema parity, SwiGLU/Top-K shape drifts, block-scale association drift, valid-extent relation drift, grouped-GEMM group-extent/address-safety drift, runtime-index dtype drift, CuTe epilogue-formula drift, argmin dtype drift, the unsupported KDA `tanh.approx.f32` contract and a Target-admitted wrong-kind instruction are failure-capable regression gates, while the scientific-v3 Triton warp-specialized argmin failure remains lowering-ineligible |
