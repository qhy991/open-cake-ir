...
| G3 | passed | v25 32-case Corpus Gate, released content-bound Revision lock, independently written tmux-window1 approval and 48-source closure; successor preparation cannot write its own approval, accessed-Buffer bounds repair the v24 OOB lowering, schema rejects the old profile spelling, TinyGEMM2 reduction/epilogue drift blocks only its checked asset, and the earlier KDA-derived operation, dtype, relation, safety and backend negatives remain failure-capable |
| G4 | passed for migrated evidence, the bounded attribution slice and the TinyGEMM2 checked asset | common Flash-KMeans/TinyGEMM receipts, r37/r39 parity, full artifact roles, raw r45 replay, and raw-checked no-timing NCU attribution including all correct search survivors in the v18 qualification; the v25 diagnostic successor compiles and launches all 14 generated lowerings and retains 12 passes plus two GEMM numerical failures. TinyGEMM2 v2 binds reproduced input/oracle bytes and retained parent output; its v29 shared-B200 successor passes one current checked-asset launch with zero fallback and no timing |
...
| G6 | passed | 398 pytest items cover authoring-schema/Corpus parity, strict route/metadata/prompt boundaries, KDA-derived primitives and negatives, accessed-Buffer mask ownership, batched tie-audit failure semantics, frozen v24/v25 observation accounting, TinyGEMM2 materialization, CPU-oracle replay, parent/oracle separation and frozen current observation replay, inventory-bound Executor witnesses, backend preflight and checked-asset semantics, external-approval refusal/success transitions, clone-time custody versus replay, closed template/frozen revision bindings, candidate-set transport/selection/replay, event tampering, reference-bundle retention, reasoning-effort qualification, observable lowering generation, preorder tie abstention, retained historical/current decisions, every correct search survivor's attribution, two-part Estimand missingness, tool-rich lifecycles, Claim-Scope routing, external Campaign custody, Lab authority, GPU tutorial custody and all three matched Claim Scopes |
