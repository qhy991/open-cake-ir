...
| G3 | passed | v21 29-case Corpus Gate, released content-bound Revision lock, reviewed approval and 45-source closure; authoring-schema parity, SwiGLU/Top-K shape drifts, block-scale association drift, valid-extent relation drift, grouped-GEMM group-extent/address-safety drift, argmin dtype drift, the unsupported KDA `tanh.approx.f32` contract and a Target-admitted wrong-kind instruction are failure-capable regression gates, while the scientific-v3 Triton warp-specialized argmin failure remains lowering-ineligible |
