...
Only candidate-set `matched_search` with `claim_scope=artifact_optimization_only`
and the closed matched event vocabulary admits this policy. Scientific comparison,
system qualification, portfolio and direct CUDA do not. A stable Study template
...
The delivered v51 FMA/GEMM models remain outside this path: their templates, input
contracts, Kineto timer/cache context and runtime do not match the Flash assay. Positive
