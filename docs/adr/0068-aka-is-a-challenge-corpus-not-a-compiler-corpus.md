# ADR 0068: AKA is a challenge corpus, not a Compiler Corpus

Status: accepted for external-corpus assessment, 2026-08-28. Renumbered from 0038 on
2026-09-18; that number was already held by
[0038-qsa-needs-stateful-selection-and-launch-composition.md](0038-qsa-needs-stateful-selection-and-launch-composition.md).
The AKA campaign is closed (owner decision D7, 2026-09-18); its runners and tests are on
the `history` branch ([ADR 0067](0067-retired-release-outputs-live-on-the-history-branch.md))
and the published dataset under `docs/data/` with its verification tools stays on `main`.
