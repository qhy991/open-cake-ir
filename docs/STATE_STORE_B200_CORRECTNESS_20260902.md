...
## V5 source-bound Compiler v41 successor — 2026-09-06

The current example now prepares `open-cake-state-store-b8x128-b200-correctness-v5`
from the released v41 Compiler source at
`ee32b76870c4166c8143e8774c37c1212a0b6e1c`. Preparation verifies that exact Git lock,
its full released source closure and the executing Compiler's checkout before
assessing or lowering. Its provenance names the verified Compiler commit; a later
Compiler revision is refused and requires a deliberate task successor.

The same four workloads and numerical judge are retained, with isolated v5 task and
deployment identities. Only CPU preparation and static acceptance have been checked;
**v5 GPU correctness is pending**. No v5 GPU submission, sanitizer, timing or provider
run was performed, and v4's target result is not a v41 result. V1-v4 records above are
unchanged. The historical v4 source, task and v40 Compiler can be replayed together at
Git commit `82491bb7658a4da92be444fa029b26b02635a154`.
