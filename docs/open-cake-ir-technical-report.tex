...
\usepackage{hyperref,fancyhdr,titlesec,caption,microtype,multicol}
...
\vspace{8mm}
{\noindent\sffamily\bfseries\color{ink}\fontsize{27}{38}\selectfont Agent 驱动的\\GPU Kernel 与编译器\\协同演进\par}
\vspace{7mm}
{\noindent\color{muted}\large A Technical Report on Agent-Driven GPU Kernel\\and Compiler Co-Evolution\par}
\vspace{13mm}
...
\section*{摘要}
\begin{notebox}
\noindent\textbf{问题。}GPU Kernel Agent 能提出候选实现，但一次失败或一项延迟读数很难说明问题
属于调度、表达能力、目标准入还是测量质量。
\par\smallskip
\noindent\textbf{方法。}open-cake-ir 以 typed Schedule 表达单 Kernel 的硬件决策，以 Program 组合
阶段；Compiler 给出局部化诊断，Lab 在冻结合同下搜索，Evaluation 使用外部 oracle、目标计时和
profiler 确认，Evidence 保存可审计的观察。重复缺口在运行外由后继提交修复。
\par\smallskip
\noindent\textbf{证据。}当前源码的 179 项 Corpus Gate 用例符合预期；B300、C550 已有范围明确的
设备案例。平台接入和固定任务的局部优化已有证据，跨硬件经验材料或显式变换能否降低 Agent
搜索成本，仍需预注册的四组处理实验。软件合同、设备正确性和因果收益分别验收。
\end{notebox}
