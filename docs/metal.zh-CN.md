...
已封存基线。指定 `--incumbent-registry` 时，各任务在预检阶段选择精确匹配的 incumbent 或
starter fallback，并将同一产物及其选择来源一起传递。执行阶段在 provider qualification 前
再次核对 registry；若选择已过期，则明确拒绝并保留预检产物，不改选另一份基线。
第一个任务完成真实两轮 provider qualification 后，后续任务在
