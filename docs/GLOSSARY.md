...
下面帮助你阅读正文。提到一种概念，不代表项目已经支持它的全部用法；准确限制仍看对应合同。

| 词 | 用普通话说 | 小例子或辨别方法 |
| --- | --- | --- |
| GPU / CPU | GPU 适合很多小组同时做类似计算；CPU 负责通用计算和控制工作 | CPU 准备输入，GPU 同时处理许多行 |
| 算子 / kernel | 算子是一项数学任务；kernel 是一次启动时在 GPU 上执行的程序 | 一个注意力算子可能要启动多个 kernel |
| tensor / shape | tensor 是按维度组织的一组数；shape 是每个维度的大小 | `[2,3]` 表示 2 行、每行 3 个数 |
| dtype / FP32 / BF16 | dtype 是数字的存储类型；FP32 每数 4 字节，BF16 每数 2 字节，精度不同 | 减少位数可能丢掉小数的后几位 |
| Buffer | 计划中有名字、形状、类型和存储位置的一组数据 | `x` 是输入，`x_tile` 是正在计算的小块 |
| tile | 一次拿来处理的小块数据 | 一行 128 个数，每次处理 32 个，需要 4 块 |
| thread / warp / CTA | thread 是线程；NVIDIA 的 warp 是 32 个线程；CTA 是协作的线程块 | 一个 CTA 可以有多个 warp，不能与一个数混同 |
| global / register / shared / tensor memory | 不同 GPU 存储位置：大容量显存、线程快速存储、线程块共享存储、目标专用存储 | 同样一组数放哪里，会影响容量、协作和生成方式 |
| ABI | 调用程序时双方约好的接口：参数按什么顺序、什么类型、怎样交回结果 | 直接传一个数字，与传“这个数字存放的地址”是不同接口 |
| runtime scalar / 常数 | 前者是调用时才给的一个数；后者在写计划时已固定 | 每次调用传不同学习率，不能悄悄变成写死的 `0.1` |
| broadcast / splat | broadcast 按规则对齐并复用小形状数据；splat 明确把一个值复制成多个位置 | 一个 `2` 变成 `[2,2,2]` 需要相应语义，不能只改形状标签 |
| mask / predicate | mask 决定哪些位置参与读写；predicate 是决定真假的条件 | 最后一块只有 3 个有效数时，不能越界读第 4 个 |
| state / 原地更新 | 调用者交来的状态在原位置修改，并留给后续使用 | 计数器从 5 变 6，不是另建一个输出来冒充原状态 |
| rounding / FMA | rounding 是把放不下的精确数舍入到可存的值；FMA 把乘加合在一起，只在最后舍入一次 | 先乘法舍入、再加法舍入，可能得到不同末位 |
| NaN / Inf / subnormal | NaN 表示不是普通数值的结果；Inf 是无穷；subnormal 是浮点系统中特别小的一类数 | 正确性合同需要说明怎样处理这些情况 |
| oracle / tolerance | oracle 独立算标准答案；tolerance 规定允许偏差 | 不能看到结果超标后再把误差门槛放宽 |
| static / dynamic | static 根据计划和规则检查；dynamic 看真正执行时的行为 | 能生成源码是静态边界，GPU 完整答案是另一边界 |
| benchmark / profiler | benchmark 测多长时间；profiler 观察时间和资源花在哪里 | 带 profiler 的时间不能直接当普通运行延迟 |
| 上界 / 下界 / 估计 | 上界是最多，下界是至少，普通估计不保证方向 | “至少 10”不能读成“恰好 10”；缺证据也不能填零 |
| median / CV | median 是排序后的中间值；CV 说明波动相对于平均值的大小 | 多次测量要看稳定性，不能只选最快一次 |
| freeze / successor | freeze 固定本次内容；successor 是另建后继版本 | 新版改进不覆盖旧实验的源码与结果 |
