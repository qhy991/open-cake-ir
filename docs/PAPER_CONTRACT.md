...
| Token accounting | 80M-token budget; complete receipts and exact accounting definition unavailable | Count local `provider_tokens` once: Codex reports the resumed thread's cumulative `input_tokens + output_tokens`, so each Turn contributes its difference from the prior completed total; Claude reports invocation usage directly. Cached input remains part of input and reasoning output part of output. Retain raw counters. The stopping threshold is checked between complete invocations, so one invocation may cross it; this is not a mid-invocation hard cap and is not comparable with the paper's 80M |
...
| Reference access | No low-level target implementation at clean start | `matched-search-clean-start-reference-template.json` pairs an implementation-free Schedule interface with an empty CUDA ABI starter and rejects contamination structurally. Combined with current Executor bundle retention, an auditor can inspect the actual local input. The fixture still retains 150k/`max` and is not a paper-aligned campaign |
...
| Evidence semantics | Retained evidence supports post-run audit; the paper does not publish its event schema | Executor v23 successor matched Studies declare the closed `matched_run_v1` vocabulary. Current Executor v29 reports archive content integrity separately from filesystem custody: weak clone modes permit inspection and semantic replay but cannot support a claim-bearing projection. Earlier Studies retain bounded legacy replay; no frozen bytes are reinterpreted |
