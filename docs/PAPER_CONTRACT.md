...
| Reference access | No low-level target implementation at clean start | `matched-search-clean-start-reference-v27.json` pairs an implementation-free Schedule interface with an empty CUDA ABI starter and rejects contamination structurally. Combined with v26 bundle retention, an auditor can inspect the actual local input. The fixture still retains 150k/`max` and is not a paper-aligned campaign |
