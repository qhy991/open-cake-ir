...
| Model effort | GPT-5.6-sol with `xhigh`, held fixed across arms | Reasoning effort is an explicit Authoring Environment factor whose exact value must be live-qualified and identical across arms. Current frozen local Studies use the distinct `max` level and remain task-informed package studies; a future paper-aligned successor must qualify and freeze `xhigh` |
...
| Reference access | No low-level target implementation at clean start | `matched-search-clean-start-reference-v24.json` pairs an implementation-free Schedule interface with an empty CUDA ABI starter and rejects contamination structurally. This validates only the local reference bundle; the fixture retains 150k/`max` and is not a paper-aligned campaign |
